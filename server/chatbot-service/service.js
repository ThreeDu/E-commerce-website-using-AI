...
function detectIntent(message) {
  return recommenderService.detectIntent(message);
}

function buildBehaviorProfile(userBehavior) {
  return recommenderService.buildBehaviorProfile(userBehavior);
}

function classifyQueryType(args) {
  return recommenderService.classifyQueryType(args);
}

function buildConversationFocus(args) {
  return recommenderService.buildConversationFocus(args);
}
...
async function findRecommendedProducts(message, context = {}, options = {}) {
  ...
  const behaviorProfile = recommenderService.buildBehaviorProfile(context.userBehavior);
  ...
  const queryType = recommenderService.classifyQueryType({
    normalizedMessage,
...
// compare-related functions moved to server/chatbot-service/compare.js
